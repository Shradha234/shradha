...
def display_analysis_results(result, image_filename):
    # Get image captions
    ...
    if result.objects is not None:
        print("\nObjects in image:")
        annotate_image_with_objects(result, image_filename)

    # Get people in the image
    if result.people is not None:
        print("\nPeople in image:")
        annotate_image_with_people(result, image_filename)


def annotate_image_with_objects(result, image_filename):
...
def annotate_image_with_people(result, image_filename):
    # Prepare image for drawing
    image = Image.open(image_filename)
    fig = plt.figure(figsize=(image.width / 100, image.height / 100))
    plt.axis('off')
